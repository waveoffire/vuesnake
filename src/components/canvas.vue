  ...
  methods: {
    ...
    drawsnake(ctx: CanvasRenderingContext2D) {
      ...
      for (let i = 0; i <= this.snakeWidth - 1; i++) {
        ...
        if (i == 0) {
          this.drawImageRot(
            this.SnakeHead_image,
            this.snakeCoords[i].x,
            this.snakeCoords[i].y,
            20,
            20,
            this.snakeCoords[i].r,
            ctx
          );
        } else if (i == this.snakeWidth - 1) {
          this.drawImageRot(
            this.SnakeTail_image,
            this.snakeCoords[i].x,
            this.snakeCoords[i].y,
            20,
            20,
          ...
          if (this.snakeWidth > 1 && this.snakeWidth - 1 != i) {
            if (this.snakeCoords[i].r != this.snakeCoords[i + 1].r) {
              let r = 0;
              if (
                this.snakeCoords[i + 1].r == 90 &&
                this.snakeCoords[i].r == 0
              ) {
                this.drawImageRot(
                  this.SnakeRight_image,
                  this.snakeCoords[i].x,
                  this.snakeCoords[i].y,
                  20,
                  20,
                ...
                this.snakeCoords[i + 1].r == 0 &&
                this.snakeCoords[i].r == 270
              ) {
                this.drawImageRot(
                  this.SnakeRight_image,
                  this.snakeCoords[i].x,
                ...
                this.snakeCoords[i + 1].r == 180 &&
                this.snakeCoords[i].r == 270
              ) {
                this.drawImageRot(
                  this.SnakeLeft_image,
                  this.snakeCoords[i].x,
                  this.snakeCoords[i].y,
                  20,
                  20,
    ...
    move(ctx: CanvasRenderingContext2D) {
      var inter = setInterval(() => {
        ...
        if (
          this.snakeCoords[0].x < 0 ||
          this.snakeCoords[0].x >= 500 ||
          this.snakeCoords[0].y < 0 ||
          this.snakeCoords[0].y >= 500
        ) {
          this.gameover = true;
        }
        if (
